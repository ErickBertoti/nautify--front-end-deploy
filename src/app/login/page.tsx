...
import { buildPendingRegistrationScaffold, startGoogleOAuth, syncBackendAuth } from '@/lib/auth-flow';
...
function LoginPageContent() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      setError(signInError instanceof Error ? signInError.message : 'Erro ao iniciar login com Google');
      ...
      if (supabaseError || !data.session) {
        setError(
          supabaseError?.message === 'Invalid login credentials'
            ...
            : supabaseError?.message || 'Erro ao fazer login',
        );
        setIsLoading(false);
        return;
      }

      const authStatus = await syncBackendAuth(data.session.access_token, {
        allowPendingRegistration: true,
        profileRequiredFallback: buildPendingRegistrationScaffold(data.user ?? data.session.user),
      });

      if (authStatus === AUTH_STATE_PROFILE_REQUIRED) {
        router.push(`/auth/complete-profile?next=${encodeURIComponent(redirectTarget)}`);
        return;
      }
  ...
  return (
    <div className="min-h-screen flex" style={{ background: '#0c1322' }}>
      <div className="hidden lg:flex lg:w-[58%] relative overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-[0.04]">
          <svg viewBox="0 0 400 400" className="w-[600px] h-[600px]" style={{ transform: 'rotate(-15deg)' }}>
            ...
            {[...Array(36)].map((_, index) => {
              ...
              return <line key={index} x1={x1} y1={y1} x2={x2} y2={y2} stroke="#b9c6eb" strokeWidth={index % 9 === 0 ? '1' : '0.3'} />;
            })}
          </svg>
        </div>
        ...
        <div
          ...
          style={{
            ...
            height: '300px',
            borderRadius: '50%',
            background: 'radial-gradient(circle, rgba(185,198,235,0.08) 0%, transparent 70%)',
            filter: 'blur(40px)',
          }}
        />
        <div
          ...
          style={{
            ...
            height: '400px',
            borderRadius: '50%',
            background: 'radial-gradient(circle, rgba(238,192,104,0.06) 0%, transparent 70%)',
            filter: 'blur(60px)',
          }}
        />
        ...
        <div className="relative z-10 flex flex-col justify-between py-12 px-16 w-full">
          <div className="flex items-center gap-3 animate-fade-up">
            <Image src="/logo-white.png" alt="Nautify" width={48} height={48} className="drop-shadow-lg" />
            <div>
          ...
          <div className="flex gap-4 animate-fade-up" style={{ animationDelay: '200ms' }}>
            {[
              ...
              >
                <div className="p-2 rounded-lg transition-colors duration-300" style={{ background: 'rgba(238,192,104,0.08)' }}>
                  <Icon className="h-4 w-4 transition-colors duration-300" style={{ color: '#eec068' }} />
                </div>
                <div>
                  <p className="text-lg font-bold transition-colors duration-300" style={{ color: '#dce2f7', fontFamily: 'var(--font-heading)' }}>
                    {value}
                  </p>
      ...
      <div className="flex-1 flex items-center justify-center px-5 py-8 sm:px-8 relative" style={{ background: '#141b2b' }}>
        <div
        ...
        <div className="w-full max-w-md relative z-10">
          <div className="flex items-center gap-3 mb-10 lg:hidden animate-fade-up">
            <Image src="/logo-blue.png" alt="Nautify" width={36} height={36} />
            <h1 className="text-xl font-bold" style={{ color: '#dce2f7', fontFamily: 'var(--font-heading)' }}>
              Nautify
            </h1>
          </div>
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-6 animate-stagger">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    style={{ color: '#8f9097' }}
                    onMouseEnter={(event) => (event.currentTarget.style.color = '#dce2f7')}
                    onMouseLeave={(event) => (event.currentTarget.style.color = '#8f9097')}
                  >
              ...
              <div className="flex items-center justify-between pt-1">
                <label className="flex items-center gap-2 cursor-pointer group">
                  <div className="relative">
                    ...
                    <div className="w-4 h-4 rounded border transition-all duration-200 peer-checked:border-[#eec068] peer-checked:bg-[#eec068]/20" style={{ borderColor: 'rgba(69,71,76,0.4)' }} />
                ...
                <Link href="/esqueci-senha" className="text-sm transition-all duration-200 hover:opacity-80" style={{ color: '#eec068' }}>
                  Esqueci minha senha
                </Link>
              </div>

              {displayError && <p className="text-sm text-red-400 text-center">{displayError}</p>}
              {displayInfoMessage && <p className="text-sm text-emerald-400 text-center">{displayInfoMessage}</p>}
              ...
              <div className="flex items-center gap-3 text-[11px] uppercase tracking-[0.2em]" style={{ color: '#8f9097' }}>
                <div className="h-px flex-1 bg-[rgba(143,144,151,0.2)]" />
                ou
                <div className="h-px flex-1 bg-[rgba(143,144,151,0.2)]" />
              </div>
